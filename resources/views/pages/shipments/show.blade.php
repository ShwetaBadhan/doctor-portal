...
@section('content')
    <div class="page-wrapper">
        <div class="content">
            ...
            <div
                class="alert alert-{{ $shipment->status == 'delivered' ? 'success' : ($shipment->status == 'dispatched' ? 'primary' : 'warning') }} d-flex align-items-center mb-4">
                <i
                    class="ti {{ $shipment->status == 'delivered' ? 'ti-check' : ($shipment->status == 'dispatched' ? 'ti-truck' : 'ti-clock') }} fs-1 me-3"></i>
                <div class="flex-grow-1">
                    <h5 class="fw-bold mb-1">{{ $shipment->getStatusLabel() }}</h5>
                    <p class="mb-0 small">
                        @if ($shipment->status == 'dispatched')
                            Dispatched on {{ $shipment->dispatched_at?->format('d M Y') }}
                        @elseif($shipment->status == 'packed')
                            Packed on {{ $shipment->packed_at?->format('d M Y') }}
                        @elseif($shipment->status == 'delivered')
                            Delivered on {{ $shipment->delivered_at?->format('d M Y') }}
                        @else
                            Awaiting shipment
                        @endif
                    </p>
                ...
                <div>
                    ...
                    <form action="{{ route('shipments.update-status', $shipment) }}" method="POST" class="d-inline-flex">
                        ...
                        <select name="status" class="select form-select-sm" onchange="this.form.submit()"
                            ...
                            <option value="dispatched" {{ $shipment->status == 'dispatched' ? 'selected' : '' }}>Dispatched
                            </option>
            ...
            <div class="row">
                ...
                <div class="col-lg-8">
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <p class="fw-medium">{{ $shipment->recipient_country ?? 'India' }}</p>
                                </div>
                                <div class="col-md-4">
                    ...
                    @if ($shipment->remarks)
                        <div class="card mb-4">
                            <div class="card-header">
                                <h6 class="fw-bold mb-0"><i class="ti ti-note me-2"></i>Remarks</h6>
                            </div>
                            <div class="card-body">
                                <p class="mb-0 text-muted">{{ nl2br(e($shipment->remarks)) }}</p>
                            </div>
                        </div>
                    @endif
                </div>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                @if ($shipment->invoice)
                                    <a href="{{ route('invoices.print', $shipment->invoice) }}" class="fw-medium"
                                        target="_blank">{{ $shipment->invoice->invoice_number }}</a>
                                @else
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <ul class="timeline timeline-simple mb-0">
                                <li
                                    class="timeline-item {{ in_array($shipment->status, ['pending', 'packed', 'dispatched', 'delivered']) ? 'active' : '' }}">
                                ...
                                <li
                                    class="timeline-item {{ in_array($shipment->status, ['packed', 'dispatched', 'delivered']) ? 'active' : '' }}">
                                    <div class="timeline-dot bg-info"></div>
                                    <div class="timeline-content">
                                        <p class="fw-bold mb-0">Packed</p>
                                        <small
                                            class="text-muted">{{ $shipment->packed_at?->format('d M Y') ?? '-' }}</small>
                                    </div>
                                </li>
                                <li
                                    class="timeline-item {{ in_array($shipment->status, ['dispatched', 'delivered']) ? 'active' : '' }}">
                                    <div class="timeline-dot bg-primary"></div>
                                    <div class="timeline-content">
                                        <p class="fw-bold mb-0">Dispatched</p>
                                        <small
                                            class="text-muted">{{ $shipment->dispatched_at?->format('d M Y') ?? '-' }}</small>
                                    </div>
                                </li>
                                <li class="timeline-item {{ $shipment->status == 'delivered' ? 'active' : '' }}">
                                    ...
                                    <div class="timeline-content">
                                        ...
                                        <small
                                            class="text-muted">{{ $shipment->delivered_at?->format('d M Y') ?? '-' }}</small>
                                    </div>
                                </li>
                            </ul>
                        </div>
