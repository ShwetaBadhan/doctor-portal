                    ...
                    <tbody>
                        @forelse($appointments as $appointment)
                            <tr>
                                ...
                                <td>
                                    @if ($appointment->appointment_type == 'in_person')
                                        <span
                                            class="badge badge-soft-info rounded text-info fs-13 fw-medium">In-Person</span>
                                    @else
                                        <span
                                            class="badge badge-soft-primary rounded text-primary fs-13 fw-medium">Online</span>
                                    @endif
                                </td>
                            ...
                            <div class="offcanvas offcanvas-offset offcanvas-end" tabindex="-1"
                                id="view_details_{{ $appointment->id }}">
                                        ...
                                        <div class="px-3 my-4">
                                            <div class="row g-3">
                                                ...
                                                <div class="col-4">
                                                    ...
                                                    <span class="fw-medium">{{ $appointment->vat ?? '-' }}</span>
                                                </div>
                                                <div class="col-4">
                                                    <small class="text-muted d-block">PIT</small>
                                                    <span class="fw-medium">{{ $appointment->pit ?? '-' }}</span>
                                                </div>
                                                <div class="col-4">
                                                    <small class="text-muted d-block">Kuff</small>
                                                    <span class="fw-medium">{{ $appointment->kuff ?? '-' }}</span>
                                                </div>
                                                ...
                                                <div class="col-4">
                                                    ...
                                                    @if ($appointment->bp)
                                                        <span
                                                            class="badge bg-light text-dark">{{ $appointment->bp }}</span>
                                                    @else
                                                ...
                                                <div class="col-4">
                                                    ...
                                                    @if ($appointment->temp)
                                                        <span
                                                            class="badge bg-light text-dark">{{ $appointment->temp }}°F</span>
                                                    @else
                                                        <span class="text-muted">-</span>
                                                    @endif
                                                </div>
                                                <div class="col-4">
                                                    ...
                                                    <span class="fw-medium">{{ $appointment->pulse ?? '-' }}</span>
                                                </div>

                                                <!-- Weight, Tongue, Nails -->
                                                <div class="col-4">
                                                    <small class="text-muted d-block">Weight</small>
                                                    <span class="fw-medium">{{ $appointment->weight ?? '-' }}</span>
                                                </div>
                                                <div class="col-4">
                                                    <small class="text-muted d-block">Tongue</small>
                                                    <span class="fw-medium">{{ $appointment->tongue ?? '-' }}</span>
                                                </div>
                                                <div class="col-4">
                                                    <small class="text-muted d-block">Nails</small>
                                                    <span class="fw-medium">{{ $appointment->nails ?? '-' }}</span>
                                                </div>

                                                <!-- Cerebral Fluid -->
                                                <div class="col-4">
                                                    <small class="text-muted d-block">Cerebral Fluid</small>
                                                    @if ($appointment->cerebral_fluid)
                                                        <span
                                                            class="badge bg-{{ $appointment->cerebral_fluid == 'normal' ? 'success' : ($appointment->cerebral_fluid == 'shrink' ? 'warning' : 'danger') }} text-white">
                                                            {{ ucfirst($appointment->cerebral_fluid) }}
                                                        </span>
                                                    @else
                                                        <span class="text-muted">-</span>
                                                    @endif
                                                ...
                                                <!-- Vital Notes (Full Width) -->
                                                @if ($appointment->vital_notes)
                                                    <div class="col-12 mt-2">
                                                        <small class="text-muted d-block">Notes</small>
                                                        <p class="mb-0 text-muted small">{{ $appointment->vital_notes }}
                                                        </p>
                                                    </div>
                                                @endif
                                            </div>
                                    ...
                                    <div class="px-3 my-4">
                                        <form action="{{ route('appointments.update', $appointment->id) }}"
                                            ...
                                            <div class="row align-items-center">
                                                ...
                                                <div class="col-lg-6 col-md-6">
                                                    <div class="mb-3">
                                                        <select name="status" class="select form-select-sm"
                                                            ...
                                                            @foreach (['schedule' => 'Schedule', 'confirmed' => 'Confirmed', 'checked_in' => 'Checked In', 'checked_out' => 'Checked Out', 'cancelled' => 'Cancelled'] as $val => $label)
                                                                <option value="{{ $val }}"
                                                                    {{ $appointment->status == $val ? 'selected' : '' }}>
                                                                    {{ $label }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
